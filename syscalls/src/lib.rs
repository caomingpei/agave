...
use {
    ...
    solana_bn254::{
        prelude::{
            ...
            AltBn128Error, ALT_BN128_G1_ADD_BE, ALT_BN128_G1_MUL_BE, ALT_BN128_G1_POINT_SIZE,
        ...
        .map(|vm_slice| {
            ...
            let taint_snapshot = instrumenter.and_then(|inst| {
                vm_taint_state.and_then(|vm_state| {
                    inst.borrow().memory_taint(&vm_state.borrow(), vm_slice.ptr)
                })
            });
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        // NovaFuzz: Record PDA creation only after output write succeeds.
        if let (Some(instrumenter), Some(seed_infos)) = (instrumenter_opt.as_ref(), seed_infos_opt)
        {
            ...
            if let Some(vm_taint_state) = vm_taint_state_opt.as_ref() {
                ...
                instrumenter.borrow_mut().mark_pda_result_bytes(
                    &mut vm_taint_state,
                    address_addr,
                    derivation_id,
                    new_address,
        ...
        for _ in 0..u8::MAX {
            {
                    ...
                    Pubkey::create_program_address(&seeds_with_bump, program_id)
                {
                    ...
                    *bump_seed_ref = bump_seed[0];
                    ...
                    // NovaFuzz: Record PDA creation only after output writes succeed.
                    if let (Some(instrumenter), Some(seed_infos)) =
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        let calculation_result = match group_op {
        ...
        let result_point = match calculation_result {
...
declare_builtin_function!(
    ...
    fn rust(
        ...
        match op {
            ALT_BN128_G1_COMPRESS_BE => {
                let result_point = match alt_bn128_g1_compress_be(input) {
                    Ok(result_point) => result_point,
                    Err(e) => {
                        return if simplify_alt_bn128_syscall_error_codes {
                            Ok(1)
            ...
            ALT_BN128_G1_DECOMPRESS_BE => {
                let result_point = match alt_bn128_g1_decompress_be(input) {
                    Ok(result_point) => result_point,
                    Err(e) => {
                        return if simplify_alt_bn128_syscall_error_codes {
                            Ok(1)
            ...
            ALT_BN128_G2_COMPRESS_BE => {
                let result_point = match alt_bn128_g2_compress_be(input) {
                    Ok(result_point) => result_point,
                    Err(e) => {
                        return if simplify_alt_bn128_syscall_error_codes {
                            Ok(1)
            ...
            ALT_BN128_G2_DECOMPRESS_BE => {
                let result_point = match alt_bn128_g2_decompress_be(input) {
                    Ok(result_point) => result_point,
                    Err(e) => {
                        return if simplify_alt_bn128_syscall_error_codes {
                            Ok(1)
